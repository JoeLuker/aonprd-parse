...
class Unwrapper:
    ...
    def __init__(
        self, graph: nx.DiGraph, data: Dict[str, Any], structure: Dict[str, Any]
    ):
        self.graph = graph
        self.data = data
    ...
    def find_nodes_with_attributes(
        self, target_attributes: List[Dict[str, Any]]
    ) -> Dict[Tuple, List[str]]:
            ...
            if node_data.get("type") == "tag" and node_data.get("name") == "div":
                ...
                if attributes_id:
                    ...
                    for target in target_attributes:
                        if all(
                            node_attributes.get(k) == v
                            if not isinstance(v, list)
                            else set(node_attributes.get(k, [])) == set(v)
                            ...
                            logger.debug(
                                f"Node {node} matched target attributes: {dict(target)}"
                            )
                            break

        logger.info(
            f"Identified {sum(len(nodes) for nodes in matching_nodes.values())} nodes matching target attributes."
        )
    ...
    def rewire_graph(self, nodes_to_remove: Set[str]):
        ...
        for node in nodes_to_remove:
            if node not in self.unwrapped_graph:
                logger.warning(
                    f"Node {node} not found in graph during rewiring. Skipping."
                )
                continue
            ...
            for pred in predecessors:
                for succ in successors:
                    edge_data = {
                        **self.unwrapped_graph.edges.get((pred, node), {}),
    ...
    async def unwrap_matching_nodes(self, target_attributes: List[Dict[str, Any]]):
        ...
        nodes_to_remove = set(
            node for nodes in matching_nodes.values() for node in nodes
        )
        ...
        for node in nodes_to_remove:
            if node in self.graph.nodes:
                ...
                logger.warning(
                    f"Node {node} was not found in the original graph. It may have been removed during rewiring."
                )

        self.unwrapped_data["attributes"] = {
            k: v
            for k, v in self.unwrapped_data["attributes"].items()
            if k not in attributes_to_remove
        }
        removed_attributes = len(self.data["attributes"]) - len(
            self.unwrapped_data["attributes"]
        )
        ...
        self.unwrapped_structure = {
            "nodes": [
                {**data, "id": node}
                for node, data in self.unwrapped_graph.nodes(data=True)
            ],
            "edges": [
                {**data, "source": u, "target": v}
                for u, v, data in self.unwrapped_graph.edges(data=True)
            ],
        }
