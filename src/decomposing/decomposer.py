...
class Decomposer:
    def __init__(self):
        self.data = {"texts": {}, "doctypes": {}, "comments": {}, "attributes": {}}
    ...
    async def process(self):
        ...
        for task in tqdm.as_completed(
            tasks, total=len(tasks), desc="Processing HTML files", unit="file"
        ):
            await task
    ...
    def _process_document(self, soup: BeautifulSoup, filename: str):
        ...
        for order, element in enumerate(soup.contents, start=1):
            child_node_id, _ = self._process_node(
                element, parent_id=root_node_id, order=order
            )
    ...
    def _process_node(
        self, element: Any, parent_id: str, order: int
    ) -> Tuple[str, str]:
        if isinstance(element, Doctype):
            ...
            node_id, _ = self._create_node(
                "tag",
                name=element.name,
                attributes=element.attrs,
                children_hashes=tuple(children_hashes),
            )
            self._create_edge(
                source_id=parent_id,
                target_id=node_id,
    ...
    def _create_node(self, node_type: str, **kwargs) -> Tuple[str, str]:
        identifier = (
            node_type,
            kwargs.get("name", ""),
            make_hashable(kwargs.get("attributes")),
            kwargs.get("children_hashes", ()),
            kwargs.get("content", ""),
        )
    ...
    def _create_edge(
        self, source_id: str, target_id: str, relationship: str, order: int = None
    ):
    ...
    def _compute_subtree_hash(self, element: Any) -> Tuple[str, str]:
        if isinstance(element, Doctype):
            ...
            if element.strip():
                hash_value = self._hash_subtree(
                    node_type="textnode", content=element.strip()
                )
            else:
                return "", ""
        elif isinstance(element, Tag):
...
async def main():
    ...
    if output_dir.exists() and any(output_dir.iterdir()):
        logger.info(
            "Output directory already exists and contains files. Skipping decomposition process."
        )
        return
