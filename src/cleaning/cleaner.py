...
# Initialize Logger
logger = Logger.get_logger('CleanerLogger', config.paths.log_dir / config.logging.processor_log)
...
async def clean_and_copy_files(source_dir: Path, destination_dir: Path, skip_files: Set[str]) -> Tuple[List[str], Dict[str, int]]:
    modifications = []
    ...
    for file_path in tqdm(files, desc="Processing files"):
        ...
        try:
            ...
            modified_content, applied_replacements = FileOperations.apply_replacements(content, config.cleaning.replacements)
            if applied_replacements:
                ...
                logger.info(f"No replacements needed for file: {file_path.name}. Copied as-is.")
...
async def main():
    ...
    try:
        ...
        # Process HTML files
        processed_files = await process_html_files(config.paths.manual_cleaned_html_data)
