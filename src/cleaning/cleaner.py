...
# Initialize Logger
logger = Logger.get_logger("CleanerLogger", config.paths.log_dir / "cleaner.log")
...
async def process_file(
    filename: str, input_dir: str, url_mapping: Dict[str, str]
...
async def find_duplicates(
    input_dir: str, url_mapping: Dict[str, str], uf: UnionFind
    ...
    try:
        ...
        logger.info(
            f"Found {len(file_hashes)} unique files based on hashes and relative URLs."
        )
...
async def find_additional_duplicates(
    input_dir: str,
    relative_url_mapping: Dict[str, List[Tuple[str, str]]],
    uf: UnionFind,
    file_hashes: Dict[str, Tuple[str, str]],
    ...
    try:
        ...
        logger.info(
            f"Loading contents of {len(tasks)} files for similarity comparison..."
        )
        file_contents_list = await tqdm.gather(*tasks, desc="Loading file contents")
        file_contents = {
            fname: content for fname, content in file_contents_list if content
        }

        # Define a helper function for similarity comparison
        async def compare_similarity(
            canonical_file: str, duplicate_file: str
            ...
            try:
                ...
                similarity = await asyncio.to_thread(
                    rapidfuzz.fuzz.ratio, canonical_content, duplicate_content
                )
                if similarity >= SIMILARITY_THRESHOLD:
                    logger.debug(
                        f"Duplicate found: {duplicate_file} is {similarity}% similar to {canonical_file}"
                    )
                    return (canonical_file, duplicate_file)
                return None
            except Exception as e:
                logger.error(
                    f"Error comparing {canonical_file} and {duplicate_file}: {e}"
                )
                return None
                ...
                for dup_file, _ in files[1:]:
                    if uf.find(canonical_file) != uf.find(dup_file):
                        comparison_tasks.append(
                            compare_similarity(canonical_file, dup_file)
                        )

        comparison_results = await tqdm.gather(
            *comparison_tasks, desc="Comparing file similarities"
        )
        ...
        logger.info(
            f"Identified {len(duplicate_pairs)} additional duplicate pairs based on similarity."
        )
...
async def parse_html_file(
    file_path: str, url_mapping: Dict[str, str]
) -> Optional[Dict]:
    """
    ...
    try:
        ...
        _, _, _, relative_url = await process_file(
            file_name, os.path.dirname(file_path), url_mapping
        )

        form = soup.find("form")
        form_action = form.get("action") if form else ""
        form_action_decoded = (
            unquote_plus(form_action).replace("./", "") if form_action else None
        )
...
async def process_html_files(
    input_dir: str, url_mapping: Dict[str, str], unique_files: Set[str]
) -> List[Dict]:
    """
...
async def insert_data(
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            for i in tqdm(
                range(0, len(processed_files), BATCH_SIZE), desc="Inserting files"
            ):
            ...
            for i in tqdm(
                range(0, len(processed_files), BATCH_SIZE), desc="Inserting meta tags"
            ):
                batch = processed_files[i : i + BATCH_SIZE]
                meta_entries = []
                for file_data in batch:
                    ...
                    if file_id:
                        meta_entries.extend(
                            [
                                ...
                                for name, content in file_data["meta_tags"]
                            ]
                        )
                if meta_entries:
                    await cursor.executemany(
            ...
            for i in tqdm(
                range(0, len(processed_files), BATCH_SIZE),
                desc="Inserting external links",
            ):
                batch = processed_files[i : i + BATCH_SIZE]
                link_entries = []
                for file_data in batch:
            ...
            if canonical_entries:
                for i in tqdm(
                    range(0, len(canonical_entries), BATCH_SIZE),
                    desc="Inserting canonical mappings",
                ):
...
async def save_deduplicated_files(
    input_dir: str, output_dir: str, unique_files: Set[str]
):
    """
    ...
    try:
        ...
        async with aiosqlite.connect(crawler_db_path) as conn:
            ...
            async with conn.execute(
                'SELECT url FROM urls WHERE status = "completed"'
            ) as cursor:
                async for row in cursor:
                    url = row[0]
                    file_hash = get_url_hash(url)
                    file_name = (
                        f"{file_hash}.html"  # Assuming file names have .html extension
                    )
...
async def determine_unique_files(
    uf: UnionFind, file_hashes: Dict[str, Tuple[str, str]]
) -> Set[str]:
    """
    ...
    try:
        ...
        unique_files: Set[str] = set(
            file_hashes[f_hash][0]
            for f_hash in file_hashes
            if file_hashes[f_hash][0] not in all_duplicates
        )
        return unique_files
    except Exception as e:
...
async def main():
    ...
    try:
        ...
        if OUTPUT_DIR.exists() and any(OUTPUT_DIR.iterdir()):
            logger.info(
                "Output directory already exists and contains files. Skipping processing."
            )
            return

        # Manage output directory and database connection
        async with managed_directory(OUTPUT_DIR) as output_dir, managed_database(
            DB_NAME
        ) as conn:
            ...
            file_hashes, relative_url_mapping = await find_duplicates(
                INPUT_DIR, url_mapping, uf
            )
            logger.info(
                f"Found {len(file_hashes)} unique files based on hashes and relative URLs"
            )

            await find_additional_duplicates(
                INPUT_DIR, relative_url_mapping, uf, file_hashes, url_mapping
            )
            ...
            processed_files = await process_html_files(
                INPUT_DIR, url_mapping, unique_files
            )
