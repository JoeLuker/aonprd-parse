...
# Initialize Logger
logger = Logger.get_logger('ManualCleaningLogger', config.paths.log_dir / config.logging.processor_log)
...
async def copy_file_async(source: Path, destination: Path):
    ...
    try:
        ...
        logger.error(f"Failed to copy file from {source} to {destination}: {e}", exc_info=True)
        raise

async def process_file_async(source_path: Path, destination_path: Path, skip_files: Set[str]) -> Tuple[bool, Set[str]]:
    ...
    try:
        ...
        if applied_replacements:
            ...
            await copy_file_async(source_path, destination_path)
            logger.info(f"No replacements needed for file: {source_path.name}. Copied as-is.")
            return False, set()
    except Exception as e:
        logger.error(f"Error processing file {source_path.name}: {e}", exc_info=True)
        return False, set()

async def clean_and_copy_files_async(skip_files: Set[str]) -> Tuple[List[str], Dict[str, int]]:
    ...
    for result in tqdm(asyncio.as_completed(tasks), total=len(tasks), desc="Processing files"):
...
async def prepare_canonical_mapping(mapping_data: List[Dict[str, Any]]) -> List[Tuple[str, str, str, str]]:
    """Prepare canonical mapping tuples for database insertion."""
    canonical_mappings = [
        (item['file_without_sort'], item['file_with_sort'], item['url_without_sort'], item['url_with_sort'])
        for item in mapping_data
    ]
    logger.debug(f"Prepared {len(canonical_mappings)} canonical mappings")
    return canonical_mappings

async def insert_canonical_mapping(conn: sqlite3.Connection, data_to_insert: List[Tuple[str, str, str, str]]):
    ...
    try:
        ...
        """, data_to_insert)
        await conn.commit()
        logger.info(f"Inserted {len(data_to_insert)} entries into canonical_mapping table")
    except sqlite3.Error as e:
