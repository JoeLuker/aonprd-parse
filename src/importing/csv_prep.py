...
# Initialize Logger
logger = Logger.get_logger('CSVPrepLogger', config.paths.log_dir / config.logging.csv_prep_log)
...
class CSVExporter:
    ...
    def _initialize_csv_files(self) -> Dict[str, Dict[str, Any]]:
        ...
        file_specs = {
            ...
            'HAS_DOCTYPE': ('has_doctype.csv', ['source', 'target', 'source_type', 'target_type', 'order']),
            'HAS_COMMENT': ('has_comment.csv', ['source', 'target', 'source_type', 'target_type', 'order']),
            'CONTAINS_TEXT': ('contains_text.csv', ['source', 'target', 'source_type', 'target_type', 'order']),
            'CONTAINS_TAG': ('contains_tag.csv', ['source', 'target', 'source_type', 'target_type', 'order']),
            'HAS_ROOT': ('has_root.csv', ['source', 'target', 'source_type', 'target_type', 'order']),
            'HAS_ATTRIBUTE': ('has_attribute.csv', ['source', 'target', 'source_type', 'target_type'])
        }
    ...
    def export_node(self, node_type: str, node_id: str, properties: Dict[str, Any]):
        ...
        if not writer:
            ...
            return
        if node_type == 'Tag':
            writer.writerow([node_id, properties.get('name', ''), properties.get('attributes_id', '')])
        else:
    ...
    def export_attribute(self, attribute_id: str, attributes: Dict[str, Any]):
        ...
        if not writer:
            logger.warning(f"No CSV writer found for attributes")
            return
        writer.writerow([attribute_id, json.dumps(attributes)])
        logger.debug(f"Exported attribute: {attribute_id}")

    def export_relationship(self, rel_type: str, source: str, target: str, source_type: str, target_type: str, order: int = None):
        """
...
class CSVPrep:
    ...
    """
    def __init__(self, structure: Dict[str, Any], data: Dict[str, Any], exporter: CSVExporter):
            ...
            if node_type in ['Document', 'Doctype', 'Comment', 'TextNode', 'Tag']:
                try:
                    if node_type == 'Document':
                        ...
                        self.exporter.export_node('Document', node_id, {"filename": filename})
                    elif node_type == 'Doctype':
                        doctype_content = self.data['doctypes'].get(node.get('data_id', ''), '')
                        self.exporter.export_node('Doctype', node_id, {"doctype": doctype_content})
                    elif node_type == 'Comment':
                        comment_content = self.data['comments'].get(node.get('data_id', ''), '')
                        self.exporter.export_node('Comment', node_id, {"comment": comment_content})
                    elif node_type == 'TextNode':
                        text_content = self.data['texts'].get(node.get('data_id', ''), '')
                        self.exporter.export_node('TextNode', node_id, {"content": text_content})
                    elif node_type == 'Tag':
                        tag_name = node.get('name', '')
                        attributes_id = node.get('attributes_id', '')
                        self.exporter.export_node('Tag', node_id, {"name": tag_name, "attributes_id": attributes_id})
                        if attributes_id:
                            self.exporter.export_relationship('HAS_ATTRIBUTE', node_id, attributes_id, 'Tag', 'Attribute')
                except Exception as e:
    ...
    def prepare_attributes(self):
        ...
        for attribute_id, attributes in tqdm(self.data.get('attributes', {}).items(), desc="Exporting Attributes"):
            try:
    ...
    def prepare_relationships(self):
        ...
        for edge in tqdm(self.structure.get('edges', []), desc="Exporting Relationships"):
            try:
                ...
                if csv_rel_type:
                    if csv_rel_type == 'HAS_ATTRIBUTE':
                        self.exporter.export_relationship(csv_rel_type, source, target, source_type, 'Attribute')
                    else:
                        self.exporter.export_relationship(csv_rel_type, source, target, source_type, target_type, order)
                else:
...
def main():
    ...
    # Define paths
    structure_pickle_path = config.paths.condensed_output_dir / f"unwrapped_{config.files.structure_pickle}"
    data_pickle_path = config.paths.condensed_output_dir / f"unwrapped_{config.files.data_pickle}"
