...
# Initialize Logger
logger = Logger.get_logger('MemgraphImporterLogger', config.paths.log_dir / config.logging.memgraph_importer_log)
...
class MemgraphImporter:
    def __init__(self, csv_dir: Path, batch_size: int = 1000, max_workers: int = 16):
        ...
        self.connection_pool = MemgraphConnectionPool(config.memgraph.host, config.memgraph.port, max_workers)
    ...
    async def process_relationship_batch(self, batch: List[Dict[str, Any]], relationship_type: str, source_type: str, target_type: str):
        query = f"""
        UNWIND $batch AS rel
    ...
    async def import_relationships(self, csv_file_path: Path, relationship_type: str):
        ...
        for i in range(0, total_relationships, self.batch_size):
            ...
            tasks.append(self.process_relationship_batch(batch, relationship_type, source_type, target_type))

        for f in atqdm(asyncio.as_completed(tasks), total=len(tasks), desc=f"Importing {relationship_type}"):
            await f

        logger.info(f"Imported {total_relationships} relationships for {relationship_type}")
...
async def main():
    ...
    importer = MemgraphImporter(csv_dir, batch_size=config.memgraph.batch_size, max_workers=config.processing.max_workers)
